      ...
      <div className="container mx-auto px-4 mt-8">
        ...
        <div className="max-w-6xl mx-auto space-y-24">
          {phanBonTypes.map((item, idx) => {
            let modelsForType = [];
            if (idx === 0) modelsForType = phanBonModels.filter(m => m.modelUrl.includes('phan_dam'));
            else if (idx === 1) modelsForType = phanBonModels.filter(m => m.modelUrl.includes('phan_lan'));
            else if (idx === 2) modelsForType = phanBonModels.filter(m => m.modelUrl.includes('phan_kali'));

            return (
              <div key={idx} className="space-y-10 animate-fade-in-up" style={{ animationDelay: `${idx * 150}ms` }}>

                {/* Info Card - Full width */}
                <div
                  className="glass-panel rounded-3xl p-8 md:p-10 border relative overflow-hidden card-hover-lift"
                  style={{
                    borderColor: `rgba(${item.rgb}, 0.25)`,
                  }}
                >
                  {/* Background Glow */}
                  <div className="absolute top-0 right-0 w-64 h-64 rounded-full blur-[100px] pointer-events-none opacity-50" style={{ backgroundColor: `rgba(${item.rgb}, 0.15)` }} />
                  ...
                  <div className="flex flex-col lg:flex-row gap-8 relative z-10">
                    ...
                    <div className="w-full lg:w-1/3 flex flex-col justify-center">
                      <div className="flex items-center gap-4 mb-2">
                        ...
                        >
                          <span className={`${item.title.includes("NPK") ? "text-xl tracking-widest" : "text-2xl"} font-black text-white text-center leading-tight`}>
                            {item.element.match(/\((.*?)\)/)?.[1] || (item.title.includes("NPK") ? "NPK" : item.element)}
                          </span>
                        </div>
                        <div>
                    ...
                    {/* Right: Details Grid */}
                    <div className="w-full lg:w-2/3 grid sm:grid-cols-2 gap-5">
                      <div
                      ...
                      >
                        <h3 className="text-sm font-bold uppercase tracking-wider mb-3" style={{ color: item.color }}>Thành phần chính</h3>
                        <p className="text-[var(--text)] opacity-90 leading-relaxed font-medium">
                          {item.ingredients}
                        </p>
                        {item.note && (
                          <div className="mt-4 text-sm text-[var(--muted)] flex gap-2 items-start border-t border-[rgba(255,255,255,0.05)] pt-4">
                      ...
                      <div
                        className="rounded-2xl p-6 border border-[rgba(255,255,255,0.05)] bg-[rgba(255,255,255,0.02)] h-full"
                      >
                        <h3 className="text-sm font-bold uppercase tracking-wider mb-3 text-[var(--muted)]">Tác dụng với cây</h3>
                ...
                {modelsForType.length > 0 && (
                  <div className="flex flex-col gap-8 xl:pl-8">
                    <h3 className="text-2xl font-extrabold text-[var(--text)] flex items-center gap-3 ml-2">
                      <span className="w-1.5 h-6 rounded-full" style={{ backgroundColor: item.color }}></span>
                      Mô hình 3D các hợp chất
                    </h3>
                    <div className="flex flex-col gap-8">
                      {modelsForType.map((model, mIdx) => (
                        <div
                          key={mIdx}
                          className={`rounded-2xl overflow-hidden glass-panel border chem-shimmer-border animated-border-card flex flex-col ${mIdx % 2 !== 0 ? "lg:flex-row-reverse" : "lg:flex-row"}`}
                          style={{ borderColor: model.borderColor }}
                        >
                          <div
                            className={`relative w-full lg:w-[55%] h-[22rem] flex items-center justify-center overflow-hidden bg-gradient-to-br from-black/5 to-transparent ${mIdx % 2 !== 0 ? "lg:border-l border-b lg:border-b-0" : "lg:border-r border-b lg:border-b-0"}`}
                            style={{
                          ...
                          <div className="w-full lg:w-[45%] p-8 flex flex-col justify-center items-center text-center">
                            <span
                            ...
                            <h2 className="text-4xl md:text-5xl font-extrabold mb-3" style={{ color: model.color }}>
                              {model.name}
                            </h2>
